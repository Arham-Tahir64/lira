...
## Current increment: Phase 1 foundation and task vertical slice
...
1. Invitations/acceptance, roster management, owner-transfer invariants and revocation locks; teams and project leads.
...
- UI edits status; the API also supports detail/priority/assignee/due-date edits. UI assignment, comments, uploads, and invitations remain Phase 2.
...
- Membership mutations must take an exclusive organization advisory lock matching the shared authorization lock in `withTenant`. This avoids grant escalation merely to use SQL row locks during bootstrap reads.
